...
// Hint: Remember, this is an array of objects. This means you'll need to use both array methods and object concepts to complete this. Also remember that some methods return new copies of arrays instead of changing their values in place.

  var filtered = tagMap.filter(function(item, i){
      return item.Language === "en";
    });  
    //Loops through each "item" in tagMap. In this case, they're Objects, so you can check them by their properties.
    // So, into a new ... array??? I return the items whose Language attribute is equal to "en" 
    
    // for (var prop in filtered) {
    //   console.log("key: " + prop + ", value: " + filtered[prop]);
    // }    DONT DO THIS, because filtered was returned as an array. I think...

  for (var i = 0; i < filtered.length; i++) {
    for (var prop in filtered[i]) {
      console.log("key: " + prop + ", value: " + filtered[i][prop]);
    }
  }
