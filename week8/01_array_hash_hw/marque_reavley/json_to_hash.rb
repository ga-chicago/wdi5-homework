#  INTRO TO RUBY AND WORKING WITH HASHES
#  =====================================

#  Require gems and JSON
#  ---------------------
require 'rubygems'
require 'json'

#  Introduce JSON into the mix
#  ---------------------------
...
#  Prints out heading for article list
#  -----------------------------------
puts " "
puts "The Articles I Think Are Great And So You Should Read Them Too".upcase
puts "=============================================================="
puts " "

#  Loops through the JSON and picks out the articles with 
json_array.each do |article|
  if article[:id].even?
    puts "#{article[:title].upcase} (article ID: #{article[:id]})"
    puts ("=" * article[:title].length)
    puts article[:author]
    puts ("-" * article[:author].length)
    puts article[:body]
    puts " "
    puts " "
    puts " "
